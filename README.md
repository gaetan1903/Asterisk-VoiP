...
> Quoi? , attends! Tu veux dire que ce truc n'est qu'un devoir? alors pourquoi le publier ici? :open_mouth:

Et oui, on appelle plutôt cela chez nous, à  l'ESTI **un projet**.
C'est quelque chose que beaucoup d'entreprises recherchent.

#### Ce Projet consiste à installer et configurer un serveur Asterisk
Le but de ce projet est de réaliser des appels locaux sans frais, juste un réseau commun. 
...
Dans ce petit tutoriel `Si on peut l'appeler comme ça` Nous allons utiliser Debian comme OS.

> Debian ? Mais il existe beaucoup de versions ! Lequel ?
...
Hahaha! Je plaisante, à vous de voir. Je ne pense pas être le mieux placé pour répondre à cette question . :grin:
...
donc notre instructions deviennent: 
...
Ouvrez maintenant script1.php et importez le agi-php en question 
